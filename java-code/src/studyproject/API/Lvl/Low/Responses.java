...
/**
 * Class that handles sending the responses to another client, the responses are
...
public class Responses {
	...
	/**
	 * responds to the getInfoUp call and writes the timestamp and the fileInfos
	 * in the way that is specified in the specification to the provided
	 * outputstream
	 * 
	 * @param socketStream
	 ...
	 */
	public static int respondInfo(BufferedOutputStream socketStream,
			long timestamp, ArrayList<FileInfo> fileInfos) {
		try {
			if (timestamp == 0) {
				socketStream.write((ALL_FLAG + timestamp + " "
						+ fileInfos.size() + "\n").getBytes());
				socketStream.flush();
			} else {
				socketStream.write((UPDATE_FLAG + timestamp + " "
						+ fileInfos.size() + "\n").getBytes());
				socketStream.flush();
			}
			for (FileInfo fileInfo : fileInfos) {
				socketStream.write((fileInfo.fileAction.toString() + " "
						+ fileInfo.checksum + " " + fileInfo.size + " "
						+ fileInfo.fileName + "\n").getBytes());
				socketStream.flush();
			}
		} catch (IOException e) {
	...
	/**
	 * responds to the getFile call and writes the contents of the file to which
	 * the fileStream is given to the stream. The function only writes the
	 * contents of the file between start and endIndex to the stream
	 * 
	 * @param socketStream
	 *            stream to write to
	 * 
	...
	/**
	 * responds to the getSendPermission call by sending OK and then waiting to
	 * receive the bytes of the file that this method just gave permission to be
	 * sent
	 * 
	 * @param socket
	 ...
	 * 
	 * @return 0 if transfer was completed or an error value
	 */
	public static int respondSendPermission(Socket socket,
