...
public class LogAreaHandler extends Handler {
     ...
     *
     * @param level        the LogLevel which the Handler shall take care of
     * @param toLogTo      the TableView where the Error Shall be logged, needs to be an
     *                     TableView<Error>
     * @param logAreaModel the model which holds the Boolean Properties of the checkboxes
     */
    ...
    /**
     * Extracts the VirtualFlow from the LogAreas skinProperty, the VirtualFlow is used for scrolling reasons
     */
    ...
    /**
     * Publishes a record if the record is a instance of {@link Error}, isLoggable(which checks appropiate log level)
     * and shouldBeLogged which checks for the users settings, which messages shall be logged. If the size of the
     * log messages exceeds MAX_LOG_MSG_COUNT an entry is deleted before another one is added. If an entry was removed
     * the virtual flow will scroll up for one entry to keep track of the current focused part of the messages.
     *
     * @param record the log record which shall be published
     */
    @Override
     ...
     * moment.
     *
     * @param record The LogRecord received by the publish function which is
